...
describe('Groups (Grupos)', () => {
  ...
  it('toggles a group enabled/disabled', () => {
    ...
    page.openCreateModal();
    page.fillGroupForm({ name, uniqueName: `GRP_TGL_${suffix}`, description: 'Toggle test', enabled: true });
    page.submitForm();
    cy.wait('@create');
    cy.wait('@list');
    page.assertRowVisible(name);
  ...
  it('bulk deletes multiple groups', () => {
    ...
    page.openCreateModal();
    page.fillGroupForm({ name: grp1, uniqueName: `BULK_G1_${suffix}`, description: 'Bulk test', enabled: true });
    page.submitForm();
    cy.wait('@create');
    cy.wait('@list');

    page.openCreateModal();
    page.fillGroupForm({ name: grp2, uniqueName: `BULK_G2_${suffix}`, description: 'Bulk test', enabled: true });
    page.submitForm();
    cy.wait('@create');
    cy.wait('@list');
