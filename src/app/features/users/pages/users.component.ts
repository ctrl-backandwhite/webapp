...
export class UsersComponent implements OnInit, OnDestroy {
  ...
  confirmDelete() {
    ...
    this.deleteError.set('');
    this.saveSub?.unsubscribe();
    this.saveSub = this.usersService.delete(target.id)
      .pipe(take(1))
      .subscribe({
        next: () => {
          this.deleting.set(false);
  ...
  submitUser() {
    ...
    if (this.isEditMode() && this.editingUserId() !== null) {
      this.saveSub = this.usersService.update(this.editingUserId() as number, payload)
        .pipe(take(1))
        .subscribe({
    ...
    this.saveSub = this.usersService.create(payload)
      .pipe(take(1))
      .subscribe({
        next: () => this.finishSave(),
        error: () => this.handleSaveError(),
